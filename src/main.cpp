...
int main()
{
    ...
    {
        // this can be modified by imgui, so render each frame
        // tell GLFW to capture our mouse per frame
        ...
        shaderSSAO.setFloat("screenWidth", width);
        shaderSSAO.setFloat("screenHeight", height);
        ...
        glClear(GL_COLOR_BUFFER_BIT);
        shaderSSAOBlur.use();
        shaderSSAOBlur.setInt("blurRadius", ssaoBlurRadius);
        glActiveTexture(GL_TEXTURE0);
        glBindTexture(GL_TEXTURE_2D, ssaoColorBuffer);
        renderQuad();
        glBindFramebuffer(GL_FRAMEBUFFER, 0);
        ...
        glClear(GL_COLOR_BUFFER_BIT | GL_DEPTH_BUFFER_BIT);
        shaderLightingPass.use();
        shaderLightingPass.setBool("bypass", ssaoBypass);
        shaderLightingPass.setBool("skip", ssaoSkip);
